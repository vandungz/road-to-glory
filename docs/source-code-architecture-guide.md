...
- Tự ý sinh hoặc mock điểm số bảng xếp hạng (League Table), số trận ra sân (Apps), chỉ số ẩn cầu thủ (Hidden Stats), cơ hội chuyển nhượng (Transfer Offers), đề cử Ballon d'Or, chuỗi hành trình cúp (Cup Journeys), hoặc tự tính toán lại OVR sau tiến hóa thuộc tính (Stats Evolution) ở Frontend; tất cả các logic nghiệp vụ (business logic) cốt lõi của game này bắt buộc phải được giả lập và kiểm chứng qua Backend Service/Server Action (bao gồm cả việc lọc đối thủ Cup Journeys chính xác theo quốc gia/địa lý/cúp châu lục từ DB, và việc xác thực OVR mới qua `evolvePlayerStatsAction` sau mỗi mùa giải).
