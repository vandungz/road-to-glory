...
function OfferCard({ offer, isProcessing, onAccept }: { offer: ContractOfferCard; isProcessing: boolean; onAccept: () => Promise<void> | void }) {
  ...
  return (
    <article className={`football-transfer-offer${isRenewal ? " is-renewal" : ""}`}>
      <div className="football-transfer-offer__topline"><span className="football-eyebrow">{isRenewal ? "Đề nghị gia hạn" : "Đề nghị chuyển nhượng"}</span><strong>{formatTransferFee(offer.transferFee)}</strong></div>
      <h3>{offer.clubName}</h3>
      <p className="football-transfer-offer__league">{offer.leagueName} · {offer.reason}</p>
      <div className="football-transfer-offer__details">
...
function ShortlistCard({ club, selectedDeal, isProcessing, rejected, onDealChange, onApproach }: { club: ShortlistClubCard; selectedDeal: TransferFeeDealOption; isProcessing: boolean; rejected?: { chance: number; reason: string }; onDealChange: (option: TransferFeeDealOption) => void; onApproach: () => void }) {
  ...
  return (
    <article className={`football-transfer-target${rejected ? " is-rejected" : ""}`}>
      <div className="football-transfer-target__heading"><div><h3>{club.clubName}</h3><p>{club.leagueName} · Độ phù hợp {club.prestige}/5</p></div><strong>{club.expectedLeagueApps} trận</strong></div>
      <div className="football-transfer-target__meta"><span>Phí đề xuất <b>{options[selectedDeal].fee ? formatTransferFee(options[selectedDeal].fee) : "Miễn phí"}</b></span><span>Lương đề xuất <b>{formatEuroThousands(club.previewWage)}/năm</b></span></div>
      {rejected ? <p className="football-transfer-feedback is-error">Đã từ chối · {rejected.reason}</p> : club.blockReason ? <p className="football-transfer-feedback is-error">{club.blockReason}</p> : (
        <div className={`football-transfer-target__actions ${dealTone}`}><select aria-label={`Mức phí đề xuất cho ${club.clubName}`} value={selectedDeal} disabled={isProcessing} onChange={(event) => onDealChange(event.target.value as TransferFeeDealOption)}><option value="discount">Giá mềm · tăng cơ hội</option><option value="standard">Giá thị trường</option><option value="premium">Giá cao · giảm cơ hội</option></select><Button size="sm" disabled={!canApproach} onClick={onApproach}>Ngỏ lời</Button></div>
      )}
    </article>
  );
}
...
export function PersistentTransferSection({ market, currentClubId, currentClubName, proactiveRenewalRejected = false, willingToMove, setWillingToMove, isProcessing, onAcceptOffer, onRejectAll, onApproachShortlist, onProactiveRenewal, onSearchClubs, approachRejects, approachBanner, onClose, acceptedOffer, onClearAcceptedOffer, onSelectOffer }: PersistentTransferSectionProps) {
  ...
  const pendingContent = pendingOffer && wageOptions ? (
    <section className="football-transfer-panel football-transfer-negotiation">
      <button type="button" className="football-transfer-back" onClick={() => { setPendingOffer(null); void onClearAcceptedOffer?.(); }}>← Về danh sách lời đề nghị</button>
      <div className="football-transfer-panel__heading"><span className="football-eyebrow">Bước 2 · đàm phán lương</span><h2>Chốt hợp đồng với {pendingOffer.clubName}</h2><p>Phí chuyển nhượng đã được CLB chấp thuận. Mức lương cuối cùng vẫn cần được đồng ý.</p></div>
      <div className="football-transfer-offer__details"><span><small>Phí chuyển nhượng</small><strong>{pendingOffer.transferFee ? formatTransferFee(pendingOffer.transferFee) : "Miễn phí"}</strong></span><span><small>Thời hạn</small><strong>{pendingOffer.contractYears} mùa</strong></span></div>
      <div className="football-transfer-mood" aria-label={`Mức độ vui vẻ của ${pendingOffer.clubName}: ${wageMood}%`}><div><small>Mức độ vui vẻ của CLB</small><strong>{wageMood}%</strong></div><div className="football-transfer-mood__track"><span style={{ width: `${wageMood}%`, backgroundColor: wageMood <= 50 ? "var(--football-accent)" : "var(--football-positive)" }} /></div><p>{wageMood === 100 ? "CLB vẫn đang sẵn sàng thương lượng." : "Một phương án đã bị từ chối. Nếu thanh cạn, thương vụ sẽ bị hủy."}</p></div>
      <div className="football-transfer-wage-options">{(["lower", "standard", "higher"] as WageDealOption[]).map((option) => { const chance = approachChancePercent(computeWageAgreementChance({ option, baseWage: pendingOffer.wageAnnual, clubPrestige: pendingOffer.prestige, leagueTier: pendingOffer.leagueTier, expectedLeagueApps: pendingOffer.expectedLeagueApps })); const failed = failedWageOptions.includes(option); return <button type="button" key={option} disabled={failed || isProcessing} aria-disabled={failed} className={`${wageDeal === option ? "is-active" : ""}${failed ? " is-rejected" : ""}`} onClick={() => setWageDeal(option)}><span>{failed ? "Đã bị từ chối" : wageOptions[option].label}</span><strong>{formatEuroThousands(wageOptions[option].wageAnnual)}/năm</strong><small>{failed ? "Không thể chọn lại" : `${option === "lower" ? "Dễ được chấp nhận hơn" : option === "higher" ? "Có thể bị từ chối" : "Cân bằng"} · Cơ hội ${chance}%`}</small></button>; })}</div>
  ...
  const cancelledContent = cancelledOffer ? (
    <section className="football-transfer-panel football-transfer-negotiation football-transfer-negotiation--cancelled">
      <span className="football-eyebrow">Đàm phán kết thúc</span>
  ...
  const content = (
    <section className="football-transfer-window">
      <header className="football-transfer-window__header"><div><span className="football-eyebrow">Cửa sổ chuyển nhượng · {contract.seasonsLeftInCareer} mùa còn lại</span><h1>{inbound.length > 1 ? `${inbound.length} lời đề nghị` : "Cửa sổ chuyển nhượng"}</h1><p>Chọn hướng đi tiếp theo cho sự nghiệp của bạn.</p></div>{onClose && <button type="button" className="football-icon-button" onClick={onClose} aria-label="Đóng cửa sổ chuyển nhượng"><X aria-hidden="true" size={18} /></button>}</header>
      <div className="football-transfer-window__context"><span><small>Đang ở</small><strong>{isUnemployedMarket ? "Cầu thủ tự do" : currentClub}</strong></span><span><small>Lương hiện tại</small><strong>{formatEuroThousands(contract.currentWageAnnual)}/năm</strong></span><span><small>Hợp đồng còn lại</small><strong>{isUnemployedMarket ? "Không có HĐ" : `${contract.yearsRemaining}/${contract.yearsTotal} mùa`}</strong></span><span><small>Giá trị thị trường</small><strong>{formatEuroThousands(contract.marketValue)}</strong></span><label className={willingToMove ? "is-active" : ""}><input type="checkbox" checked={willingToMove} disabled={isProcessing || isUnemployedMarket} onChange={(event) => setWillingToMove(event.target.checked)} /> Sẵn sàng lắng nghe đề nghị</label></div>
      {approachBanner && <p className="football-transfer-feedback is-banner" role="status">{approachBanner}</p>}
      <nav className="football-transfer-tabs" aria-label="Các lựa chọn chuyển nhượng"><button type="button" className={activeTab === "renewal" ? "is-active" : ""} onClick={() => setActiveTab("renewal")}>Ở lại {visibleRenewal ? "· đề nghị" : ""}</button><button type="button" className={activeTab === "inbound" ? "is-active" : ""} onClick={() => setActiveTab("inbound")}>Lời đề nghị <b>{visibleInbound.length}</b></button><button type="button" className={activeTab === "shortlist" ? "is-active" : ""} onClick={() => setActiveTab("shortlist")}>Tìm CLB chủ động <b>{visibleSearchClubs.length}</b></button></nav>

      <div className="football-transfer-window__body">
        {activeTab === "renewal" && <div className="football-transfer-panel"><div className="football-transfer-panel__heading"><span className="football-eyebrow">Lựa chọn an toàn</span><h2>Tiếp tục tại {currentClub}</h2><p>Giữ vai trò, mức lương và môi trường hiện tại cho mùa giải tiếp theo.</p></div>{visibleRenewal ? <div className="football-transfer-renewal"><div><span>Đề nghị gia hạn</span><strong>{visibleRenewal.contractYears} mùa</strong></div><div><span>Lương mới</span><strong>{formatEuroThousands(visibleRenewal.wageAnnual)}/năm</strong></div><div><span>Vai trò dự kiến</span><div className="football-transfer-renewal__role"><strong>{visibleRenewal.expectedLeagueApps >= 25 ? "Đá chính" : "Xoay vòng"}</strong><small>· {visibleRenewal.expectedLeagueApps} trận/mùa</small></div></div><Button size="lg" disabled={isProcessing || proactiveRenewalRejected} loading={isProcessing} onClick={() => void onProactiveRenewal("standard")}>{proactiveRenewalRejected ? "Đề nghị đã bị từ chối" : "Đàm phán gia hạn"}</Button></div> : <p className="football-transfer-empty">CLB hiện tại chưa gửi đề nghị gia hạn. Bạn có thể xem lời đề nghị khác hoặc tìm CLB mới.</p>}</div>}
        {activeTab === "inbound" && <div className="football-transfer-panel"><div className="football-transfer-panel__heading"><span className="football-eyebrow">Các CLB đang hỏi mua</span><h2>Chọn lời đề nghị phù hợp</h2><p>Chọn lời đề nghị trước, sau đó chốt lương ở bước cuối.</p></div><div className="football-transfer-offers">{visibleInbound.length > 0 ? visibleInbound.map((offer) => <OfferCard key={`${offer.clubId}-${offer.kind}`} offer={offer} isProcessing={isProcessing} onAccept={() => beginWageNegotiation(offer)} />) : <p className="football-transfer-empty">Chưa có CLB nào gửi đề nghị mùa này.</p>}</div></div>}
        {activeTab === "shortlist" && <div className="football-transfer-panel"><div className="football-transfer-panel__heading"><span className="football-eyebrow">Chủ động định hướng</span><h2>Tìm CLB phù hợp</h2><p>Chọn mức phí ở bước mở lời; lương sẽ được đàm phán sau khi CLB đồng ý.</p></div><div className="football-transfer-search"><label><Search aria-hidden="true" size={16} /><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Tìm theo tên CLB..." /></label><select aria-label="Lọc theo giải đấu" value={league} onChange={(event) => { setLeague(event.target.value); setPage(1); }}><option value="all">Tất cả giải đấu</option>{search.leagues.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select aria-label="Lọc theo danh tiếng" value={prestige} onChange={(event) => { setPrestige(event.target.value); setPage(1); }}><option value="all">Mọi mức danh tiếng</option><option value="5">5 sao</option><option value="4">4 sao</option><option value="3">3 sao</option><option value="2">2 sao</option><option value="1">1 sao</option></select></div>{isSearching ? <p className="football-transfer-empty">Đang tìm CLB...</p> : visibleSearchClubs.length > 0 ? <div className="football-transfer-targets">{visibleSearchClubs.map((club) => <ShortlistCard key={club.clubId} club={club} selectedDeal={feeDeals[club.clubId] ?? "standard"} isProcessing={isProcessing} rejected={approachRejects[club.clubId]} onDealChange={(option) => setFeeDeals((previous) => ({ ...previous, [club.clubId]: option }))} onApproach={() => void onApproachShortlist(club, feeDeals[club.clubId] ?? "standard")} />)}</div> : <p className="football-transfer-empty">Không tìm thấy CLB phù hợp với bộ lọc.</p>}{totalPages > 1 && <div className="football-transfer-pagination"><Button size="sm" variant="quiet" disabled={page <= 1 || isSearching} onClick={() => setPage((value) => value - 1)}>Trước</Button><span>Trang {page}/{totalPages}</span><Button size="sm" variant="quiet" disabled={page >= totalPages || isSearching} onClick={() => setPage((value) => value + 1)}>Sau</Button></div>}</div>}
      </div>
      <footer className="football-transfer-window__footer"><span>{isFreeAgent ? "Bạn đang là cầu thủ tự do." : "Không chọn lời đề nghị nào? Bạn có thể ở lại CLB hiện tại."}</span><Button variant="outline" disabled={isProcessing} onClick={onRejectAll}>{isFreeAgent ? "Ký hợp đồng tự do" : "Bỏ qua · ở lại CLB"}</Button></footer>
    </section>
  );

  if (pendingContent || cancelledContent) {
    const negotiationContent = pendingContent ?? cancelledContent;
    return onClose ? <Modal open title="Đàm phán hợp đồng" onClose={onClose} size="lg" className="football-transfer-modal"><ModalHeader className="sr-only">Đàm phán hợp đồng</ModalHeader>{negotiationContent}</Modal> : negotiationContent;
  }
  if (!onClose) return content;
  return <Modal open title="Cửa sổ chuyển nhượng" onClose={onClose} size="lg" className="football-transfer-modal"><ModalHeader className="sr-only">Cửa sổ chuyển nhượng</ModalHeader>{content}</Modal>;
}
