...
function ShopItemRow({
  ...
  return (
    <div className={`rtg-shop-item rtg-shop-item--${status.tone}`}>
      <div className="rtg-shop-item__copy">
        <div className="rtg-shop-item__title">
          <span>{item.name}</span>
          {status.tone === "active" && <small>Đang hoạt động</small>}
          {status.tone === "locked" && <small>Chưa mở bán</small>}
        </div>
        <p>{item.description}</p>
        {error && <span className="rtg-shop-item__error" role="alert">{error}</span>}
      </div>

      <div className="rtg-shop-item__action">
        <span className="rtg-shop-item__price">{formatEuroThousands(item.priceThousands)}</span>
        {status.tone === "available" ? (
          ...
          <span className="rtg-shop-item__status">{status.label}</span>
        )}
      </div>
    </div>
  );
...
export function ShopModal({
  ...
  const content = (
    <div className={`rtg-shop-content${standalone ? " rtg-shop-content--standalone" : ""}`}>
      <header className="rtg-shop-content__header">
        <div className="rtg-shop-content__heading">
          <span className="rtg-eyebrow">
            {canPurchase ? `Áp dụng cho mùa tuổi ${targetSeason}` : "Cửa hàng theo mùa"}
          </span>
          <h3>{standalone ? "Cửa hàng" : "Cửa hàng đầu mùa"}</h3>
          {standalone && (
            <p className="rtg-shop-content__subtitle">
              Tiền lương và tiền chuyển nhượng đã vào ví. Mỗi vật phẩm mua một lần, và chỉ tác động cho đúng mùa giải đó.
            </p>
          )}
        </div>
        <div className="rtg-shop-content__balance">
          <span>{formatEuroThousands(walletBalance)}</span>
          <small>Số dư ví</small>
        </div>
        {!embedded && onClose && (
          <button type="button" className="rtg-icon-button rtg-shop-content__close" onClick={onClose} aria-label="Đóng">
            <X aria-hidden="true" size={19} strokeWidth={1.6} />
          </button>
        )}
      </header>

      <div className="rtg-shop-content__body">
        <div className={`rtg-shop-content__section-heading${standalone ? " rtg-shop-content__section-heading--hidden" : ""}`}>
          <ShoppingBag aria-hidden="true" size={20} strokeWidth={1.6} />
          <span>Cửa hàng</span>
        </div>
        <p className="rtg-shop-content__intro">
          {canPurchase
        ...
        <div className="rtg-shop-content__items">
      ...
      <footer className="rtg-shop-content__footer">
  ...
  return (
    <Modal open title="Cửa hàng" onClose={() => onClose?.()} size="lg" className="rtg-shop-modal">
      {content}
    </Modal>
  );
}
